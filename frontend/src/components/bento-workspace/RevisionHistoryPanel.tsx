...
function SourceBadge({ source }: { source: string }) {
  const config = {
    ...
    system: { label: "System", bg: "bg-slate-500/10", text: "text-text-secondary", border: "border-slate-500/20" },
  }[source] ?? { label: source, bg: "bg-slate-500/10", text: "text-text-secondary", border: "border-slate-500/20" };
  ...
  if (isLoading) {
    return (
      <div className="p-10 flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-xs text-text-muted">Loading revision history...</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        <div className="size-12 rounded-xl bg-hover-bg border border-border flex items-center justify-center">
          <History className="size-5 text-text-faint" />
        </div>
        <div className="text-center">
          <p className="text-sm text-text-muted font-medium">No revision history</p>
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-3">
      ...
      {revisions.map((rev) => (
        <div
          key={rev.id}
          className="bg-hover-bg border border-border rounded-xl overflow-hidden hover:border-border-prominent transition-colors"
        >
          {/* Revision header */}
                ...
                {confirmId === rev.id ? (
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button
                      ...
                      className="text-[11px] text-text-muted hover:text-text-primary px-2 py-1 transition-colors"
                    >
                      Cancel
                    </button>
                  </div>
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-[11px] text-text-muted hover:text-indigo-400 px-2 py-1 hover:bg-indigo-500/10 rounded-md transition-all border border-transparent hover:border-indigo-500/20 shrink-0"
                  >
          ...
          <div className="px-5 pb-4">
            {rev.content ? (
              <div className="bg-surface-code border border-border rounded-lg px-4 py-3 max-h-32 overflow-hidden relative">
              ...
              <p className="text-[11px] text-text-faint italic px-1">Empty content</p>
            )}
          </div>
        </div>
      ))}
  ...
  if (revisions.length === 0) {
    return (
      <div className="px-4 py-5 text-center">
        <FileText className="size-4 text-text-faint mx-auto mb-2" />
        <p className="text-[11px] text-text-muted">No description history yet</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-h-64 overflow-y-auto custom-scrollbar">
      {revisions.slice(0, 10).map((rev) => (
        <div
          key={rev.id}
          className="px-4 py-2.5 border-b border-border last:border-0 hover:bg-hover-bg transition-colors"
        >
          ...
          {canEdit && (
            <button
              ...
              disabled={isRestoring}
              className="flex items-center gap-1 text-[10px] text-text-faint hover:text-indigo-400 transition-colors disabled:opacity-50"
            >
