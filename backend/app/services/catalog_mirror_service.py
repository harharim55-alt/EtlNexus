...
from app.integrations.spark_connect_client import SparkTableSchema, spark_connect_client
...
class CatalogMirrorService:
    ...
    async def refresh_from_spark(self) -> list[SparkTableSchema]:
        """Read all table schemas from Spark Connect and replace the mirror table.

        Returns the mirrored table schemas (each carrying an inferred ``schedule``).
        If Spark Connect returns nothing (server down / no tables), the existing
        mirror is left untouched so a transient Spark outage never wipes the cache.
        """
        try:
