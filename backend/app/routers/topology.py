...
async def get_pipeline_topology(
    ...
    result = await service.build_pipeline_topology(pipeline_id, dag_id)
    if result is None:
...
async def get_upstream_topology(
    ...
    # Cache check after resolving dag_id happens inside the service,
    # but we can do a quick check for exact key match here
    ...
    service = TopologyService(session)
    result = await service.build_upstream_topology(pipeline_id, dag_id)
    if result is None:
        raise HTTPException(status_code=404, detail="Pipeline not found or has no task_id")
