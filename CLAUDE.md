...
- **Caching:** In-memory TTL caches only (process-local, `backend/app/cache.py`) — no external cache store (no Redis). `clear_all()` clears the calling process's caches after each sync/poll cycle. The APScheduler also runs per-process, so multiple backend instances each run their own sync jobs and keep independent caches that converge via TTL expiry. See `docs/adr/001-in-memory-cache-design.md`.
